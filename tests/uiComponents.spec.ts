...
test.describe('Form Layouts Page', () => {
    ...
    test('lists and dropdown', async({page}) => {
        ...
        page.getByRole('list') //can be used with UL tag
        page.getByRole('listitem') //can be used with li
    ...
    test('web tables', async({page}) => {
        ...
        const targetedRow = page.getByRole('row', {name: "11"}).filter({has: page.locator('td').nth(1).getByText('11')})
        await targetedRow.locator('.nb-edit').click()
        await page.locator('input-editor').getByPlaceholder('E-mail').clear()
        await page.locator('input-editor').getByPlaceholder('E-mail').fill('test@test.com')
        await page.locator('.nb-checkmark').click()
        await expect(targetedRow.locator('td').nth(5)).toHaveText('test@test.com')
...
test('sliders', async({page}) => {
    //update attributes coordinates shortcut
